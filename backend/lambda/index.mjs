...
export async function handler(event) {
  ...
  if (path === '/rewards' && method === 'GET') return json(200, { rewards: await rewardSlots(userId, session.developerMode), meta: await metaReward(userId) });
  ...
  if (rewardMatch) {
    ...
    const completions = await completionStatus(userId); if (!unlocked(reward, completions) && !(session.developerMode && ['reward-01', 'reward-02', 'reward-05'].includes(reward.id))) return json(403, { error: 'Completá el Sudoku correspondiente para abrir esta recompensa.' });
